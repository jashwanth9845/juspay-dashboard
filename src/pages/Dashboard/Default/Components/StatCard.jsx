...
const StatCard = ({ title, value, increasedBy, decreasedBy }) => {
  return (
    <Link className={styles?.statCard} to={"/dashboard/projects/orders"}>
      <h3 className={`${styles?.statCardHeading} semibold-14`}>{title}</h3>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8 }}
        className={styles?.statCardData}
      >
        <p className="semibold-24">{value}</p>
        {increasedBy && (
          <p className={styles.statusView}>
